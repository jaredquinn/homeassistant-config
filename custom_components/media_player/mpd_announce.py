...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_FORCE_QUEUE, default=False): cv.boolean,
})
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    force = config.get(CONF_FORCE_QUEUE)

    device = MpdDevice(host, port, password, name, force)
...
class MpdDevice(MediaPlayerDevice):
    ...
    def __init__(self, server, port, password, name, force):
