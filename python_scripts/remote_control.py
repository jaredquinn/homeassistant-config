...
value = data.get('adjustment')

logger.info('NUTES Entity ID = %s, State = %s, Command %s = %s (old: %s)' % (entity_id, state, command, selected, previous))
...
commands = {
  ...
  'pioneer': {
      ...
      'vol_down': '',
      'vol_up': ''
  },
...
if command == 'volume':
    ...
    if value < 0:
        value = abs(value)
        sval = commands['pioneer']['vol_down'] 
    else:
        sval = commands['pioneer']['vol_up'] 
